...
class Watchdog(threading.Thread):
    ...
    def run(self):
        ...
        while time.time() - start_time < self.get_max_timeout() and self.init.compile_eot==False:
            time.sleep(1)  # Check every second
        
        #ic(self.init.compile_eot) 
        #ic("max_timeout=",self.get_max_timeout())    
        #ic("line 133 ",time.time() - start_time )   
            ...
            if empty_flag:
                ...
                if self._run_queue.empty():
                    ...
                    if current_entry is not None:
                        # Print future status
                        # print(f"Future status: {current_entry['future']._state}")
                        # Check if future is done
                        if current_entry['future']._state == 'FINISHED':
                            # Move current_entry to the _done_queue
                            current_entry['status'] = 'done'
                            self._run_queue.get()
                            self._run_queue.put(current_entry)
...
def main(regression_config_file, system_config_file, compile_commands_file, run_commands_file):
    try:
        ...
        CompilationTestMonitor_obj = CompilationTestMonitor(init, system_config)
        CompilationTestMonitor_obj.set_wait_queue(init.compile_wait_queue)
        CompilationTestMonitor_obj.set_run_queue(init.compile_run_queue)
        CompilationTestMonitor_obj.set_done_queue(init.compile_done_queue)
        CompilationTestMonitor_obj.set_name("CompilationTestMonitor")
        
        CompilationWatchdog_obj = Watchdog (init, system_config)
        CompilationWatchdog_obj.set_wait_queue(init.compile_wait_queue)
        CompilationWatchdog_obj.set_run_queue(init.compile_run_queue)
        CompilationWatchdog_obj.set_done_queue(init.compile_done_queue)
        CompilationWatchdog_obj.set_name("CompilationWatchdog")
        CompilationWatchdog_obj.set_max_timeout(system_config['script']['max_timeout_compile'])
        ...
        threads = [
            CompilationTestMonitor_obj,
            CompilationSupervisor_obj,
            CompilationWatchdog_obj,
            #RuntimeTestMonitor(),
