            ...
            def handle_event(event):
                ...
                if event.receiver == "ALL" or event.receiver == name:
                    ...
                    if event.ev_instruction == "exit thread":
                        ...
                        self.test_eot = True  # Example of handling an "exit thread" event
                return
        return
...
class Watchdog(threading.Thread):
    ...
    def run(self):
        ...
        while  time.time() - start_time < max_timeout and self.update_eot()==False:
...
class TestDoneQMgr(CompDoneQMgr) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
    
    def run(self):
            empty_flag = True
            while self.config.test_eot==False:
                with self.lock:
                    self.event.listen_to_events(self._name)  # Start listening to events
                    if self.event.return_event is not None:
                            #self.event.return_event.print_event("DEBUG:"+self._name)
                        ...
                        if current_entry is not None:
                            ...
                            if current_entry['future']._state == 'FINISHED':
                                ...
                                with self.lock:
                                    ...
                                    # Reset check for empty flag
                                    self.empty_flag = False
class CompRunQMgr(CompDoneQMgr) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
            ...
            while self.config.compile_eot==False:
                with self.lock:
                    ...
                    if self._wait_queue.empty():
                        ...
                        # Peek the current first entry from _run_queue
                        current_entry = self._wait_queue.peek()
                        #ic(current_entry)
                        if current_entry is not None:
                            # Print future status
                            #print(f"Future status: {current_entry['future']._state} , return_code: {current_entry['future']._result.returncode}")
    ...
    def process_runQ(self):
        ...
        if task['priority'] > 0:
            task_get = self._wait_queue.get()
            task_get['priority'] = 0
            self._wait_queue.put(task_get) 
            ic(task_get['priority'])
            ic()

 #---\/---
class TestRunQMgr(CompDoneQMgr) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
 ...
 #---/\---
class RunQ_ArbExec(threading.Thread):
    _max_timeout = 0
    _makefile_path = ''
    ...
    def run(self):
        try:
            ...
            print(f"{self._name}:: Error in RunQ_ArbExec: {e}")
            os._exit(1)
    ...
    def process_task(self):
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                if 'priority' in task:
                    ...
                    raise ValueError("wait queue does not contain a 'priority' key")
                if  priority == 0 :
...
class TaskRunQ_ArbExec(RunQ_ArbExec):
...
def main(regression_config_file, system_config_file, compile_commands_file, run_commands_file):
    try:
        ...
        CompRunQ_ArbExec_obj = RunQ_ArbExec(config, system_config)
        CompRunQ_ArbExec_obj.set_makefile_path(system_config['script']['compile_makefile'])
        CompRunQ_ArbExec_obj.set_max_concurrent_task(system_config['script']['concurrent_compile'])
        CompRunQ_ArbExec_obj.set_wait_queue(config.compile_wait_queue)
        CompRunQ_ArbExec_obj.set_run_queue(config.compile_run_queue)
        CompRunQ_ArbExec_obj.set_done_queue(config.compile_done_queue)
        CompRunQ_ArbExec_obj.set_name("CompRunQ_ArbExec")
        ...
        CompRunQMgr_obj = CompRunQMgr(config, system_config)
        ...
        TestWatchdog_obj.set_eot_mode('test')
        
        TestRunQ_ArbExec_obj = TaskRunQ_ArbExec(config, system_config)
        TestRunQ_ArbExec_obj.set_makefile_path(system_config['script']['test_makefile'])
        TestRunQ_ArbExec_obj.set_max_concurrent_task(system_config['script']['concurrent_run'])
        TestRunQ_ArbExec_obj.set_wait_queue(config.test_wait_queue)
        TestRunQ_ArbExec_obj.set_run_queue(config.test_run_queue)
        TestRunQ_ArbExec_obj.set_done_queue(config.test_done_queue)
        TestRunQ_ArbExec_obj.set_name("TestRunQ_ArbExec")
        
        TestRunQMgr_obj = TestRunQMgr(config, system_config)
        TestRunQMgr_obj.set_wait_queue(config.test_wait_queue)
        TestRunQMgr_obj.set_run_queue(config.test_run_queue)
        TestRunQMgr_obj.set_done_queue(config.test_done_queue)
        TestRunQMgr_obj.set_name("TestRunQMgr")
        
        TestDoneQMgr_obj = TestDoneQMgr(config, system_config)
        TestDoneQMgr_obj.set_wait_queue(config.test_wait_queue)
        TestDoneQMgr_obj.set_run_queue(config.test_run_queue)
        TestDoneQMgr_obj.set_done_queue(config.test_done_queue)
        TestDoneQMgr_obj.set_name("TestDoneQMgr")

        threads = [
            CompDoneQMgr_obj,
            #CompRunQ_ArbExec_obj,
            CompRunQMgr_obj,
            CompWatchdog_obj,
            TestWatchdog_obj,
            #TestRunQ_ArbExec_obj,
            TestRunQMgr_obj,
            TestDoneQMgr_obj
            #RunTestServer()
        ]
        ...
        config.print_queue(config.test_wait_queue, "Test Wait")
        config.print_queue(config.test_run_queue, "Test Run")
        config.print_queue(config.test_done_queue, "Test Done")
