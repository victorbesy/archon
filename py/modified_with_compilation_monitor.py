...
class CompilationTestMonitor(threading.Thread):
    ...
    def run(self):
        super().run()  # Call the original run method if it exists
        while True:
            try:
                future, task_metadata = self._run_queue.get()
                result = future.result()  # Wait for the future to complete
                print(f"Task {task_metadata['id']} completed with status: {result}")
            except queue.Empty:
                # Handle empty queue condition, potentially to break the loop or simply pass
                pass
            except Exception as e:
                print(f"Error monitoring compile_run_queue task: {str(e)}")
...
class RegressionSupervisor(threading.Thread):
    ...
    def process_task(self):
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                executor.submit(subprocess.run, ["make", "-C", subdir])
