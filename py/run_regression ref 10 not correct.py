...
class BroadcastEvent:
    def __init__(self, ev_instruction, sender, receiver,QMessage):
        ...
        self._id = random.randint(1, 1000000)  # Unique ID for the message

    def print_event(self,header=''):
        print(f" {header}:: {self._id} Event: {self.ev_instruction} Sender: {self.sender}, Receiver: {self.receiver}, Message : {self.QMessage}")
    
class RunRegressionBase:
    def __init__(self):
    ...
    def listen_to_events(self,name): 
        ...
        def handle_event(event):
            ...
            if unique_event_key in self.processed_event_ids:
                return None # Exit if this event has already been processed
...
class Watchdog(threading.Thread):
    ...
    def run(self):
        ...
        while  time.time() - start_time < max_timeout and self.update_eot()==False:
            ...
            if self.config.return_event is not None:
                ic()
                self.config.return_event.print_event("DEBUG 284")
                self.config.return_event = None
    ...
    def run(self):
        try:
            ...
            while self.config.test_eot == False:
                ...
                if self.config.return_event is not None:
                     ic()
                     self.config.return_event.print_event("DEBUG!!!")
                     self.config.return_event = None
                     #self.config.test_eot = True 
                else:
                    time.sleep(1)  # Wait before checking the queue again
