...
class SmartQ(queue.Queue):
    ...
    def iter_and_remove(self):
        """
        Iterates through the queue and removes each item after processing.
        Usage:
        for item in smart_queue.iter_and_remove():
            print(item)  # This prints each item and removes it from the queue
        """
        with self.lock:
                # Create a snapshot of the queue to iterate over
                snapshot = list(self.queue)
...
class Config:    
    ...
    def extract_substring(self,input_string, delimiter, num_delimiters):
...
class CompRunQMng(CompDoneQArbEx) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
...
class ArtifactWaitQMng(CompDoneQArbEx) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
    ...
    def process_waitQ(self):
        if not self._done_queue.empty() and not self._wait_queue.empty():
            for task_done in self._done_queue.iter():
                task_done_subdir = self.config.extract_substring(task_done['subdir'], "/", 2)
                for task_wait in self._wait_queue.iter_and_remove():
                    task_wait_subdir = self.config.extract_substring(task_wait['subdir'], "/", 2)
                    if task_done_subdir == task_wait_subdir:
                        task_wait_approved  = task_wait['approved']
                        if len(task_wait_approved) > 0 :
                            task_wait_approved.remove(self._name)
                            task_wait['approved'] = task_wait_approved
                    self._wait_queue.put(task_wait)
#---\/---
class ArtifactRunQMng(CompDoneQArbEx) :
    def __init__(self, config, system_config):
        super().__init__(config, system_config)
...
def main(regression_config_file, system_config_file, compile_commands_file, run_commands_file):
    try:
        ...
        CompRunQMng_obj = CompRunQMng(config, system_config)
        ...
        ArtifactWaitQMng_obj = ArtifactWaitQMng(config, system_config)
        ArtifactWaitQMng_obj.set_wait_queue(config.test_wait_queue)
        ArtifactWaitQMng_obj.set_run_queue(config.test_run_queue)
        ArtifactWaitQMng_obj.set_done_queue(config.compile_done_queue)
        ArtifactWaitQMng_obj.set_name("ArtifactWaitQMng")
         
        ArtifactRunQMng_obj = ArtifactRunQMng(config, system_config)
        ArtifactRunQMng_obj.set_wait_queue(config.test_wait_queue)
        ArtifactRunQMng_obj.set_run_queue(config.test_run_queue)
        ArtifactRunQMng_obj.set_done_queue(config.compile_done_queue)
        ArtifactRunQMng_obj.set_name("ArtifactWaitQMng")
