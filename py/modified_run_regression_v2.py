...
def run(self):
    # Existing implementation of run() might be here. Extend it as follows:
    while not self.compile_run_queue.empty():
        future, task_metadata = self.compile_run_queue.get()
        # Wait for the future to complete
        try:
            result = future.result(timeout=None)  # Adjust timeout as needed
            print(f"Task {task_metadata['task_id']} completed with return code: {result.returncode}")
        except concurrent.futures.TimeoutError:
            print(f"Task {task_metadata['task_id']} timed out")
        except Exception as exc:
            print(f"Task {task_metadata['task_id']} raised an exception: {exc}")
