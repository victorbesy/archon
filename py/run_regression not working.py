...
class Config:    
    ...
    def find_entry_index(self,my_list, target_entry):
        for index, entry in enumerate(my_list):
            if entry == target_entry:
                return index
        return None  # Return None if the entry is not found
    
    def remove_entry_by_index(self,my_list, index):
        approved = my_list['approved']
...
class RunQArbEx(threading.Thread):
    _max_timeout = 0
    _makefile_path = ''
    ...
    def run(self):
        try:
            ...
            print(f"{self._name}:: Error in RunQArbEx: {e}")
            os._exit(1)
...
class ArtifactRunQArbEx(RunQArbEx):
...
class CompRunQMng(CompDoneQArbEx) :
            ...
            while self.config.compile_eot==False:
                with self.lock:
                    ...
                    if self._wait_queue.empty():
                        # If it's empty, wait 1 sec and go back to the beginning of the loop
                        #ic(self._wait_queue)
                        #self.config.print_queue(self._wait_queue, "Debug")
                        time.sleep(1)
                        continue
                    else:
                        # Peek the current first entry from _run_queue
    ...
    def process_runQ(self):
        task = self._wait_queue.get()
        hit = self.config.find_entry_index(task['approved'], self._name)
        if hit != None:
            result = self.config.remove_entry_by_index(task,hit)
        #self.config.print_queue(self._wait_queue, "Debug")
...
def main(regression_config_file, system_config_file, compile_commands_file, run_commands_file):
    try:
        ...
        CompRunQArbEx_obj = RunQArbEx(config, system_config)
