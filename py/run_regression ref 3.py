...
class CompilationTestMonitor(threading.Thread):
    ...
    def run(self):
     ...
     while True:
        ...
        #if self._run_queue.empty() and self._done_queue.full():  # Example condition, adjust as needed
        #    time.sleep(1)
        ...
        while True:
            if not self._run_queue.empty():
                ...
                print(f'Future Status: {current_entry["future"]._state}')
                if current_entry['future']._state == 'FINISHED':
                    self._run_queue.get()  # Remove the entry from _run_queue
                    self._done_queue.put(current_entry)  # Add it to _done_queue
            else:
                print("Run queue is empty, waiting...")
                time.sleep(1)

        while True:
            if not self._run_queue.empty():
                current_entry = self._run_queue.peek()
                # Assuming 'future status' refers to some property or method of the queue entries
                print(f"Future status: {current_entry['future']._state}")  # Adjust according to your entry structure
